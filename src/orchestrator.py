...
# ---------------------------------------------------------------------------
# flowdelta init – bootstrap FlowDelta in any project
# ---------------------------------------------------------------------------

@cli.command(name="init")
@click.argument("project_dir", default=".", type=click.Path())
@click.option("--src-dir", "src_dir", default="src", show_default=True,
              help="Relative path to the application source directory")
@click.option("--output-dir", "output_dir", default="generated_tests", show_default=True,
              help="Where generated test files will be written")
@click.option("--store-path", "store_path", default=".flowdelta/runs", show_default=True,
              help="Where FlowDelta persists traces and deltas")
@click.option("--force", is_flag=True, help="Overwrite an existing flowdelta.yaml")
def init(
    project_dir: str,
    src_dir: str,
    output_dir: str,
    ...
    """Bootstrap FlowDelta in PROJECT_DIR by creating flowdelta.yaml.

    Run this once in the root of any project you want to instrument::

        flowdelta init /path/to/my-project --src-dir src --output-dir tests/generated

    Then instrument your pipeline entry point::

        from flowdelta import FlowDelta

        fd = FlowDelta(config_path="flowdelta.yaml")
        result = fd.observe(my_main_function, *args, flow_id="my-pipeline")
    """
    out_dir = Path(project_dir).resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    cfg_file = out_dir / "flowdelta.yaml"
    if cfg_file.exists() and not force:
        console.print(
            f"[yellow]{cfg_file} already exists.[/yellow]  "
            "Pass [bold]--force[/bold] to overwrite."
        )
        return

    # Load bundled default config and patch user-supplied paths.
    builtin_cfg = Path(__file__).parent.parent / "config" / "config.yaml"
    base_cfg: dict = {}
    if builtin_cfg.exists():
        base_cfg = yaml.safe_load(builtin_cfg.read_text(encoding="utf-8")) or {}

    base_cfg.setdefault("delta_engine", {})["store_path"] = store_path
    base_cfg.setdefault("test_generator", {})["output_dir"] = output_dir
    base_cfg.setdefault("flow_identifier", {})["src_dir"] = src_dir

    header = (
        "# flowdelta.yaml — generated by `flowdelta init`\n"
        "# Edit the settings below, then instrument your code:\n"
        "#\n"
        "#   from flowdelta import FlowDelta\n"
        "#   fd = FlowDelta(config_path='flowdelta.yaml')\n"
        ...
        "#   @track(flow_id='my-pipeline')\n"
        "#   def my_pipeline(*args): ...\n\n"
    )
    cfg_file.write_text(
        header + yaml.dump(base_cfg, default_flow_style=False, sort_keys=False),
        encoding="utf-8",
    )

    # Ensure the generated tests directory exists and is importable by pytest.
    gen_dir = out_dir / output_dir
    ...
    (out_dir / store_path).mkdir(parents=True, exist_ok=True)

    console.print(
        f"[bold green]✓[/bold green] FlowDelta initialised in "
        f"[green]{out_dir}[/green]"
    )
    console.print(f"  Config  : [cyan]{cfg_file}[/cyan]")
    console.print(f"  Tests   : [cyan]{gen_dir}[/cyan]")
    console.print(f"  Store   : [cyan]{out_dir / store_path}[/cyan]")
    console.print("\n[bold]Next steps[/bold]")
    console.print("  1. Edit [cyan]flowdelta.yaml[/cyan] to set your LLM provider / API key")
    console.print("  2. Instrument your entry-point function:")
    console.print("       [green]from flowdelta import FlowDelta[/green]")
    console.print("       [green]fd = FlowDelta(config_path='flowdelta.yaml')[/green]")
    console.print("       [green]result = fd.observe(my_pipeline, *args, flow_id='my-pipeline')[/green]")
    console.print("  3. Or apply the decorator instead:")
    console.print("       [green]from flowdelta import track[/green]")
    console.print("       [green]@track(flow_id='my-pipeline', golden=True)[/green]")
    console.print("  4. Launch the dashboard:")
    console.print("       [green]flowdelta --config flowdelta.yaml dashboard[/green]")
...
@cli.command()
@click.argument("flow_id")
@click.option("--run-id", default=None, help="Specific run to generate from (default: latest golden)")
@click.option("--preview", is_flag=True, default=False,
              help="Print a preview of the test file without writing it to disk")
...
def generate(
    ...
    By default shows a preview and asks for confirmation before writing.
    Use --no-confirm to write immediately, or --preview to only preview.
    """
    pipeline: FlowDeltaPipeline = ctx.obj["pipeline"]
    if run_id:
        ...
        golden_trace = pipeline.store.load_golden(flow_id)
        if golden_trace:
            delta_data = pipeline.store.load_delta(golden_trace.get("run_id", ""))
        else:
            delta_data = None
    if not delta_data:
    ...
    from .test_generator import AssertionGenerator, LLMTestWriter
    ...
    # Build preview source without hitting the network
    spec = AssertionGenerator().generate(td)
    spec = LLMTestWriter(api_key="").augment(spec)  # heuristic only

    _pkg_root = Path(__file__).parent.parent
    env = Environment(
        loader=FileSystemLoader(str(_pkg_root / "templates")),
        undefined=StrictUndefined,
        trim_blocks=True,
        lstrip_blocks=True,
    )
    source = env.get_template("test_module.py.j2").render(
        spec=spec,
    ...
    # Always show the preview summary
    console.print(f"\n[bold cyan]Preview[/bold cyan] — [yellow]{filename}[/yellow]   "
                  f"([bold]{test_count}[/bold] test{'s' if test_count != 1 else ''})\n")

    if preview:
        # --preview: print source and stop
        console.print(source)
        return

    # Show first 40 lines as a teaser
    lines = source.splitlines()
    SHOW = 40
    for ln in lines[:SHOW]:
        console.print(f"  [dim]{ln}[/dim]")
    if len(lines) > SHOW:
        console.print(f"  [dim]... ({len(lines) - SHOW} more lines)[/dim]")
    console.print()

    if not no_confirm:
        confirmed = click.confirm(
            f"Write {filename} to {pipeline.renderer.output_dir}?",
            default=True,
        )
        if not confirmed:
    ...
    # Write to disk
    out_path = pipeline.renderer.output_dir / filename
    pipeline.renderer.output_dir.mkdir(parents=True, exist_ok=True)
    out_path.write_text(source, encoding="utf-8")
    console.print(f"[bold green]✓[/bold green] Tests written → [green]{out_path}[/green]")
